...
uint8_t read_register_buf(uint8_t reg, uint8_t* buf, uint8_t len)
{
  ...
  beginTransaction();
  status = _SPItransfer( R_REGISTER | ( REGISTER_MASK & reg ) );
  while ( len-- ){
    *buf++ = _SPItransfer(0xff);
  }
  endTransaction();
...
uint8_t read_register(uint8_t reg)
{
  ...
  beginTransaction();
  _SPItransfer( R_REGISTER | ( REGISTER_MASK & reg ) );
  result = _SPItransfer(0xff);
  endTransaction();
...
uint8_t write_register_buf(uint8_t reg, const uint8_t* buf, uint8_t len)
{
  ...
  beginTransaction();
  status = _SPItransfer( W_REGISTER | ( REGISTER_MASK & reg ) );
  while ( len-- )
    _SPItransfer(*buf++);
  endTransaction();
...
uint8_t write_payload(const void* buf, uint8_t data_len, const uint8_t writeType)
{
  ...
  beginTransaction();
  status = _SPItransfer( writeType );
  while ( data_len-- ) {
    _SPItransfer(*current++);
  }
  while ( blank_len-- ) {
    _SPItransfer(0);
  }
...
uint8_t read_payload(void* buf, uint8_t data_len)
{
  ...
  beginTransaction();
  status = _SPItransfer( R_RX_PAYLOAD );
  while ( data_len-- ) {
    *current++ = _SPItransfer(0xFF);
  }
  while ( blank_len-- ) {
    _SPItransfer(0xff);
  }
...
uint8_t radio_begin(void)
{
  ...
  uint8_t setup=0;
    ...
    // Initialize pins
  	// TODO: here initialization preiph
   // _SPI.begin();
    ce(LOW);
  	csn(HIGH);
...
void errNotify(){
	...
	failureDetected = 1; // RF24 HARDWARE FAIL: Radio not responding, verify pin connections, wiring, etc.
	//delay(5000);
}
#endif
/******************************************************************/
...
uint8_t radio_write( const void* buf, uint8_t len ){
	return radio_write_multicast(buf,len,0);
}
/****************************************************************************/
...
uint8_t writeFast( const void* buf, uint8_t len ){
	return writeFast_mcast(buf,len,0);
}
...
void startFastWrite( const void* buf, uint8_t len, const uint8_t multicast){ //TMRh20
	uint8_t startTx = 1;
...
uint8_t radio_getDynamicPayloadSize(void)
{
  ...
  spi_rxbuff[1] = 0xff;
  beginTransaction();
  _SPItransfernb( (char *) spi_txbuff, (char *) spi_rxbuff, 2);
  result = spi_rxbuff[1];
  endTransaction();
  #else
  beginTransaction();
  _SPItransfer( R_RX_PL_WID );
  result = _SPItransfer(0xff);
  endTransaction();
  #endif
...
void radio_openWritingPipe(const uint8_t address)
{
  ...
  // expects it LSB first too, so we're good.
	uint8_t address64[5] = {address,0xf1,0xf0,0xa1,0xa0};
	write_register_buf(RX_ADDR_P0,address64, addr_width);
	write_register_buf(TX_ADDR, address64, addr_width);
...
void radio_openReadingPipe(uint8_t child, const uint8_t address)
{
  ...
  // startListening() will have to restore it.
	uint8_t address64[5] = {address,0xf1,0xf0,0xa1,0xa0};
	uint8_t address8[] = {address};

  if (child == 0){
    memcpy(pipe0_reading_address,address64,addr_width);
  }
  if (child <= 6)
  {
    // For pipes 2-5, only write the LSB
    if ( child < 2 ){
    	write_register_buf(child_pipe[child], address64, addr_width);
    }else{
    	write_register_buf(child_pipe[child], address8, 1);
	}
...
void writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
  ...
  beginTransaction();
  _SPItransfer(W_ACK_PAYLOAD | ( pipe & 0x07 ) );
